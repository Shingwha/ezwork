...
SUBAGENTS = (
    ...
    "\n"
    "Long task briefs: for detailed specs (interface contracts, multi-file "
    "migrations, 30+ lines), write the brief to a UTF-8 .md file in a temp "
    "dir, then delegate with `ezwork -p \"Read the task brief at <abs path> "
    "and follow it exactly\"` — let the sub-agent read the file itself "
    "instead of inlining via `$(cat ...)`: the sub-agent inherits the bash "
    "working directory (a prior `cd` applies), so relative paths are "
    "fragile, and inlining long text risks quoting/encoding corruption "
    "(especially Chinese on Windows; build paths with `$(pwd)/file` rather "
    "than typing non-ASCII). For parallel runs use per-agent logs: "
    "`(ezwork -p \"...\" > log-a.txt 2>&1 & ezwork -p \"...\" > log-b.txt "
    "2>&1 & wait)` then tail each log. Delete briefs and logs when done.\n"
    "\n"
