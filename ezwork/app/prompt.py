...
SUBAGENTS = (
    ...
    "migrations, 30+ lines), write the brief to a UTF-8 .md file first, "
    "then run `ezwork -p \"$(cat brief.md)\"` — this avoids shell escaping "
    "and mojibake (especially Chinese on Windows), keeps the brief "
    "reviewable, and lets you launch several sub-agents in parallel with "
    "per-agent logs: `(ezwork -p \"$(cat a.md)\" > log-a.txt 2>&1 & "
    "ezwork -p \"$(cat b.md)\" > log-b.txt 2>&1 & wait)` then tail each "
    "log. Put briefs in a temp dir and delete them (with the logs) when "
    "done.\n"
    "\n"
